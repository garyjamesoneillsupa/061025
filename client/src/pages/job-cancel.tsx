...
export default function JobCancel() {
  ...
  const cancelJobMutation = useMutation({
    mutationFn: (data: { cancellationFee: string; reason: string }) =>
      apiRequest(`/api/jobs/${job.id}/cancel`, "POST", data),
    onSuccess: () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await cancelJobMutation.mutateAsync({ cancellationFee, reason });
    } finally {
    ...
    <main className="max-w-4xl mx-auto px-4 py-8">
      <div className="mb-6">
        ...
        <div className="flex items-center gap-4 mb-2">
          <h1 className="text-2xl font-bold text-gray-900">Cancel Job #{jobNumber}</h1>
          <Badge className={getStatusBgColor(job.status)}>
            {getStatusLabel(job.status)}
          </Badge>
        </div>
        <p className="text-gray-600">
          This action will permanently cancel the job and set it to "Cancelled" status.
        </p>
      </div>
        ...
        <Card className="border-orange-200 bg-orange-50/20">
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <Label htmlFor="cancellationFee" className="text-sm font-medium">
                  Cancellation Fee (£)
                </Label>
              ...
              <div className="flex gap-3 pt-4">
                ...
                >
                  {isSubmitting ? "Cancelling Job..." : "Cancel Job"}
                </Button>
              </div>
            </form>
          </CardContent>
