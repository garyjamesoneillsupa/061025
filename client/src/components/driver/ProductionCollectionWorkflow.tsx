...
interface ProductionCollectionWorkflowProps {
  job: Job;
  onBack: () => void;
  onComplete: () => void;
}
...
export default function ProductionCollectionWorkflow({ 
  job, 
  onBack, 
  onComplete 
}: ProductionCollectionWorkflowProps) {
  ...
  const updateData = (updates: Partial<CollectionData> | ((prev: CollectionData) => CollectionData)) => {
    setData(prev => {
  ...
  const saveToServerWithRetry = async (data: CollectionData, step: WorkflowStep, retryCount = 0) => {
    try {
  ...
  const saveToServer = async (data: CollectionData, step: WorkflowStep) => {
  ...
  const loadFromServer = async (jobId: string) => {
    try {
      ...
      try {
        const stored = localStorage.getItem(`collection_data_${jobId}`);
        if (stored) {
  ...
  const clearFromServer = async (jobId: string) => {
    ...
    try {
      localStorage.removeItem(`collection_data_${jobId}`);
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const loadSavedData = async () => {
      ...
      try {
        const stored = localStorage.getItem(`collection_data_${job.id}`);
        if (stored) {
          const parsed = JSON.parse(stored);
          const migratedData = {
            ...parsed.collectionData,
            expenses: parsed.collectionData?.expenses || []
          };
      ...
      if (savedData && savedData.collectionData) {
        // ⚡ CRITICAL: Data migration for old saved data
        const migratedData = {
          ...savedData.collectionData,
          // Ensure expenses property exists (for backward compatibility)
          expenses: savedData.collectionData.expenses || []
        };
    ...
    <div className="fixed inset-0 bg-gray-50 flex flex-col">
      <div className="max-w-4xl mx-auto w-full flex flex-col h-full">
        ...
        <div className="bg-white shadow-sm border-b border-gray-200 p-4 flex items-center justify-between flex-shrink-0">
          ...
          <div className="text-center">
            <h1 className="text-lg font-semibold text-gray-900">Vehicle Collection</h1>
